...
def send_email(conn, to_email, subject, html_content):
    """
    Sends one real email over SMTP (or Mailtrap API if host is mailtrap).
    ...
    if "mailtrap" in config["host"].lower():
        # Use Mailtrap Sandbox HTTP API to avoid SMTP port blocks on Render
...
def test_connection(conn):
    ...
    if "mailtrap" in config["host"].lower():
        # Verify Mailtrap API credentials
        api_token = "05143fef7739de018f2a3f9f5dd9dbb1f9c92cdd"
